...
defmodule Typo.Image.PNG do
  ...
  defp check_crc!(type, data, crc),
    do:
      :erlang.crc32([type, data]) != crc &&
        raise(Typo.ImageError, "PNG image appears to be corrupt (CRC check failed)")
  ...
  defp chunk(<<l::32, "IEND", cdata::binary-size(l), crc::32>>, png) do
    ...
    :data_error -> raise Typo.ImageError, "PNG image appears to be corrupt (data error)"
    :stream_error -> raise Typo.ImageError, "PNG image appears to be corrupt (compression error)"
  end
  ...
  defp chunk(_, _),
    do: raise(Typo.ImageError, "PNG image appears to be corrupt (possibly truncated)")
  ...
  def process!(_), do: raise(Typo.ImageError, "PNG image appears to be corrupt (invalid header)")
