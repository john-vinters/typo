...
defmodule Typo.Image.JPEG do
  ...
  def process!(_), do: raise(Typo.ImageError, "binary doesn't appear to be a JPEG image")

  # processes JPEG file segments, extracting information.
  ...
  defp process_segments(jpeg, <<255::8, type::8, rest::binary>>) do
    case type do
      ...
      t when t in [0xD9, 0xDA] -> raise Typo.ImageError, "unsupported JPEG image type"
      _other -> process_segments(jpeg, skip_segment(rest))
    end
  end

  defp process_segments(_, _), do: raise(Typo.ImageError, "JPEG image appears to be corrupt")

  # extracts information from supported SOF segments.
  @spec process_sof(binary(), binary()) :: JPEG.t()
  def process_sof(jpeg, <<_l::16, p::8, h::16, w::16, c::8, _r::binary>>),
    do: %JPEG{
      width: w,
      height: h,
  ...
  def process_sof(_, _), do: raise(Typo.ImageError, "JPEG image appears to be corrupt")

  # converts component count to a colour space.
  @spec to_colour_space(byte()) :: :DeviceCMYk | :DeviceGray | :DeviceRGB
  defp to_colour_space(1), do: :DeviceGray
  defp to_colour_space(3), do: :DeviceRGB
  defp to_colour_space(4), do: :DeviceCMYK

  defp to_colour_space(c),
    do: raise(Typo.ImageError, "unsupported JPEG image colour space: #{inspect(c)}")

  # skips a JPEG image segment.
