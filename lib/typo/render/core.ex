...
defmodule Typo.Render.Core do
  ...
  # extracts the page list.
  @spec get_page_list(PDF.t()) :: [Types.page_number()]
  defp get_page_list(%PDF{pages: p}) do
    Enum.flat_map(Map.keys(p), fn item ->
      case item do
        {:page, page} -> [page]
        _ -> []
      end
  ...
  defp get_page_resources(ctx, pdf, %Page{uuid: u}) do
    images = IdMap.get_object_use(pdf.images, u)
  ...
  # returns a box defining page size.
  @spec mediabox(Types.page_size()) :: [number()]
  defp mediabox({w, h}), do: [0, 0, w, h]

  @doc """
  Renders PDF document `pdf` with `options`.
  """
  @spec render(PDF.t(), Keyword.t()) :: iodata()
  def render(%PDF{images: images} = pdf, options \\ []) when is_list(options) do
    Context.new(options)
    |> Context.set_page_list(get_page_list(pdf), @pagetree_chunk_size)
    |> Context.set_image_list(IdMap.get_object_use(images))
    |> reserve_oids(pdf)
    |> render_header()
    |> render_pages(pdf)
    |> render_images(pdf)
  ...
  # renders PDF document catalogue object.
  ...
  # renders an image alpha channel.
  ...
  defp render_images(ctx, pdf) do
    ...
    |> Enum.reduce(ctx, fn image_id, context ->
      i = IdMap.fetch_item!(pdf.images, image_id)
  ...
  defp render_page(ctx, %PDF{defaults: defaults} = pdf, page) do
    resources = %{:Resources => get_page_resources(ctx, pdf, page)}
  ...
  # renders the page stream for an individual page.
  @spec render_page_stream(Context.t(), Page.t()) :: Context.t()
  defp render_page_stream(ctx, page),
    do: Context.object(ctx, {:page_contents, page.page}, to_iodata(page, ctx))
  ...
  defp render_pages(ctx, pdf) do
    ...
    |> Enum.reduce(ctx, fn page, context ->
      uuid = Map.fetch!(pdf.pages, {:page, page})
      p = Map.fetch!(pdf.objects, uuid)
  ...
  # renders pagetree nodes
  @spec render_pagetree(Context.t(), PDF.t()) :: Context.t()
  defp render_pagetree(ctx, pdf) do
  ...
  # renders the pagetree root node.
  ...
  # renders PDF trailer structure.
  @spec render_trailer(Context.t()) :: Context.t()
  defp render_trailer(ctx) do
    dict =
      %{
  ...
  defp reserve_image_oids(ctx, %PDF{images: images}) do
    ...
    |> Context.get_image_list()
    |> Enum.reduce(ctx, fn image_id, context ->
      i = IdMap.fetch_item!(images, image_id)
  ...
  def reserve_pagetree_oids(ctx) do
    ctx
    |> Context.get_chunks()
