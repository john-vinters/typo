...
defmodule Typo.Render.Context do
  ...
  """
  @spec allocate_tag(Context.t(), Types.tag()) :: Context.t()
  ...
  """
  @spec get_chunk!(Context.t(), Types.page_number()) :: chunk_number()
  ...
  """
  @spec get_page_list(Context.t()) :: [Types.page_number()]
  def get_page_list(%Context{page_list: page_list}), do: page_list

  @doc """
  Returns the chunked list of page numbers.
  """
  @spec get_page_list_chunked(Context.t()) :: [[Types.page_number()]]
  ...
  """
  @spec get_tag_oid!(Context.t(), Types.tag()) :: Types.oid()
  ...
  def new(options \\ []) when is_list(options), do: %Context{options: options}

  @doc """
  Outputs a PDF object with the given iodata `data`.

  `tag` is used to lookup any previously reserved oid for the object.
  If no oid is found, then a new oid is allocated for the object.
  """
  @spec object(Context.t(), Types.tag(), iodata()) :: Context.t()
  def object(%Context{offset: ofs, ofs_map: ofs_map, oid: oid, oid_map: oid_map} = ctx, tag, data)
  ...
  @doc """
  Sets the offset for a given oid/tag `key` to the current offset.
  """
  @spec set_offset(Context.t(), Types.oid() | Types.tag()) :: Context.t()
  ...
  """
  @spec set_page_list(Context.t(), [Types.page_number()], chunk_size()) :: Context.t()
  def set_page_list(%Context{} = ctx, pages, chunk_size)
      when is_list(pages) and is_integer(chunk_size) and chunk_size > 0,
